 ...
 * ---------------------------------------------------------------
 * • Runs all GEMM optimization strategies sequentially to demonstrate
 * the performance progression for a technical presentation.
 ...
 ***********************************************************************/
 ...
 void layout_transformer(TransformerModel *M)
 {
     ...
     M->layers = malloc(sizeof(TrulyOptimalLayer)); // Only need one layer struct for offsets
     ...
     // Allocate 4 separate output buffers for the benchmark
 ...
 static size_t bytes_needed(int d_model, int ctx)
 {
     size_t C = align_up(d_model, CACHE_ALIGN / sizeof(float));
     size_t T = ctx;
     size_t input_size = T * C;
     size_t weight_size = 3 * C * C;
     size_t bias_size = 3 * C;
     size_t output_size = 3 * T * C;
     
     // Input + Weights + Bias + 4 output buffers
     size_t total_floats = input_size + weight_size + bias_size + (4 * output_size);
 ...
 int main(int argc, char **argv)
 {
     ...
     size_t need_bytes = bytes_needed(C, T);
     printf("⚙  Requested model  d_model=%d  ctx=%d\n", C, T);
     printf("→ Benchmark will allocate ≈ %.2f GiB\n", need_bytes / (1024.0*1024.0*1024.0));
